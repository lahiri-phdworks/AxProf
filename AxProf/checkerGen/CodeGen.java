...
public class CodeGen {
  ...
  /*
   * Print the specified number of indents Python requires correct indentation
   */
  ...
  /*
   * Generate expression code for given expression AST node It prints the
   * appropriate code and returns a string containing an expression that will
   * contain the evaluation result after the printed code is executed by AxProf If
   * returnPVal parameter is true, it will return the p value of the expression,
   * assuming it is a statistical test If returnPVal parameter is false, it will
  ...
  public String genExpCode(AST.ASTNode exp, int indents, boolean returnPVal) {
    ...
    if (exp instanceof AST.forall) {
      ...
      // comparisons are handled differently if they contain a probabilistic
      // expression
  ...
  /*
   * Generate curve fit function expression assuming the expression is not at the
   * top level
   */
  ...
  /*
   * Generate curve fit function expression for given dataExp Maintains a list of
   * variables used in the expression (vars) topExpression indicates if the
   * expression for which code is being generated is the top expression Also keeps
   * track of the number of temporary variables created
   */
  ...
  /*
   * Generate all necessary functions Generates a per run function for per run
   * checkers Generates a per input function for per input checkers Generates a
   * final function to dump time and memory usage data Generates aggregators for
   * time, space, and accuracy data
   */
  ...
  public void generate() {
    ...
    System.out
        .println("  AxProf.dumpObtainedData(spaces,'outputs/%FILENAME%-spaceData.txt',paramNames,dataName='space')");
    if (spec.timeExp != null) {
      ...
      // System.out.println("
      // AxProf.visualizeOutput(times,'outputs/%FILENAME%-timeData.png',paramNames,/*tbd*/,dataName='time')");
      System.out.println(
    ...
    if (spec.spaceExp != null) {
      ...
      // Future work: fully autmoatic visualization generation
      // System.out.println("
      // AxProf.visualizeOutput(spaces,'outputs/%FILENAME%-spaceData.png',paramNames,/*tbd*/,dataName='space')");
      System.out.println(
